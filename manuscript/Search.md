...
Tic-Tac-Toe is a two-player, **zero-sum**, perfect-information game: whatever is good for one player is exactly as bad for the other, and both players see the full board. For such games John von Neumann's minimax theorem (1928) guarantees that a well-defined optimal value exists for every position, the score each player can force if both play perfectly. *(Dear reader, some personal history: my father knew and worked with John van Neumann at the Institutue for Advanced Study in Princeton. My father said that von Neuman would work late into the night (like some modern programmers!) and he liked to play German classic music loudly in his office.)* The **Minimax** algorithm computes that value directly. On our turn (the maximizing player) we pick the move with the highest score. On the opponent's turn (the minimizing player) we assume they pick the move with the lowest score for us. Applied recursively down to terminal positions, this yields the strongest move we can make against an opponent who never errs.
