...
To identify duplicate positions in the search tree (transpositions), we use **Zobrist Hashing**. A transposition is the same position reached by different move orders: play the same four moves in a different sequence and you often land on an identical board. If we recognize that, we can reuse the result we already computed instead of searching the position again. Zobrist's 1970 scheme, published by Albert Zobrist, gives each position a 64-bit fingerprint. Each square-piece combination, active side, castling right, and en passant file is mapped to a unique random 64-bit number, and the position's hash is the XOR of all the numbers that currently apply.
